...
| Exclude rows missing the sample-defining status fields. | The baseline sample is defined by observed Title IV and sector status. Rows with missing `PSET4FLG` or `SECTOR` cannot be assigned to the estimand without imputation. | Local sample rule; IPEDS-METHOD. | Treat this as sample definition, not data cleaning evidence about institution quality. |
...
| Keep FTFT and all-undergraduate estimands separate. | FTFT outcomes stay close to the pricing and aid cohort. Undergraduate-wide aid fields describe a broader population with different persistence and packaging histories. | IPEDS-SFA; FSA-PACK. | Do not describe all-undergraduate checks as the same model with a larger denominator. |
...
| Treat IPEDS year as a reporting index. | COA, SFA, admissions, and finance components do not all describe the same calendar moment. Year effects absorb common reporting-year shocks, but timing differences remain a design limitation. | IPEDS-METHOD; component source rules. | Do not describe year `t` estimates as same-day behavioral responses across components. |
...
| Limit estimates to the observed 2009-2023 panel. | Later aid-packaging rules may matter for policy context, but they are outside the current data window. | Local panel window; FSA-PACK | Do not describe the estimates as evidence on post-2023 rules without extending the panel and design. |
| Reserve external-shock designs for journal expansion. | A causal incidence paper would need variation beyond the current IPEDS-only fixed-effects layer, such as state appropriations or local housing-cost shocks merged to pre-period exposure. | BORUSYAK-HULL-JARAVEL; GOLDSMITH-PINKHAM-SORKIN-SWIFT | Do not make the WEAI version depend on unbuilt external-shock designs. |
